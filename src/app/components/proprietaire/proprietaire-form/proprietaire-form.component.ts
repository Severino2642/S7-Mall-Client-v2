...
@Component({
  ...
  styleUrl: './proprietaire-form.component.css'
})
export class ProprietaireFormComponent {
  @Input() item?: ProprietaireModel | null; // Données à modifier (null pour création)
  @Output() onSubmit = new EventEmitter<ProprietaireModel>();
  ...
  async createItem(formData: BoxeModel): Promise<void> {
    ...
    this.router.navigate([`owner/details/${res._id}`]);
    this.loading = false;
  }
  ...
  async updateItem(formData: BoxeModel): Promise<void> {
    ...
    this.router.navigate([`owner/details/${this.id}`]);
    this.loading = false;
  }
