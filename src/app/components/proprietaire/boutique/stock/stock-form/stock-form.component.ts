...
export interface FilleModel extends StockDetailsModel {
  selected?: boolean;
}
...
export class StockFormComponent {
  // Mere
  @Input() item?: StockModel | null;
  @Output() onSubmit = new EventEmitter<StockModel>();
  ...
  listTypeStock: string[] = ['Entrer', 'Sortie'];

  // Filles
  @Input() initialFilles: StockDetailsModel[] = [];
  filles: FilleModel[] = [];
  selectAll = false;
  ...
  initForm(): void {
    this.boxeForm = this.fb.group({
      idTypeMvtStock: [this.item?.idTypeMvtStock || '', [Validators.required]],
  ...
  async createItem(formData: any,filles:any): Promise<void> {
    ...
    this.router.navigate([`boutique/stock/details/${res._id}`]);
    this.loading = false;
  }
  ...
  async updateItem(formData: any,filles:any): Promise<void> {
    ...
    this.router.navigate([`boutique/stock/details/${this.id}`]);
    this.loading = false;
  }
  ...
  getTitre(): string {
    return this.isEditMode ? "Modification d'un mouvement de stock" : "Saisie d'un mouvement de stock";
  }
  ...
  get idTypeMvtStock() {
    return this.boxeForm.get('idTypeMvtStock')?.value;
  }

  onChangeTypeStock() {
    this.filles.forEach(fille => {
      if (this.idTypeMvtStock === 'Entrer') {
        fille.sortie = 0;
      }
      else if (this.idTypeMvtStock === 'Sortie') {
        fille.entree = 0;
      }
      console.log('Type de stock changé:', this.idTypeMvtStock);
    });
  }
}
