...
@Component({
  selector: 'app-payment-loyer-encaissement-liste',
  standalone: true,
  imports: [CommonModule, RouterLink],
  templateUrl: './payment-loyer-encaissement-liste.component.html',
  styleUrl: './payment-loyer-encaissement-liste.component.css'
})
export class PaymentLoyerEncaissementListeComponent {
  loading = false;
  errorMessage = '';
  @Input() idMere : any = "";
  // Objet Miova2
  ...
  async loadItems(): Promise<void> {
    ...
    if(res){
      this.items = res.filter(item => item.debit||0 > 0);
    }
    this.loading = false;
  }
