...
@Component({
  selector: 'app-offre-location-onglet',
  standalone: true,
  imports: [CommonModule, RouterLink],
  templateUrl: './offre-location.onglet.component.html',
  styleUrl: './offre-location.onglet.component.css'
})
export class OffreLocationOngletComponent {
  loading = false;
  errorMessage = '';
  @Input() idMere : any = "";
  // Objet Miova2
  ...
  async loadItems(): Promise<void> {
    this.loading = true;
    var res = await this.itemService.getByIdBoxe(this.idMere);
    if (res!=null){
      this.items = res;
    }
    this.loading = false;
